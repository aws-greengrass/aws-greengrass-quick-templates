...
public class CloudOps {
    ...
    public String getBucket() {
        String b = bucket;
        if (b == null) {
            bucket = b = prefs.get("bucket", null);
            if (b == null)
                b = "gg2-recipes-" + generateRandomString(10);
            System.out.println("[Using S3 bucket " + bucket + "]");
        }
        setBucket(b);
        return b;
    }
    ...
    static final char[] rsChars = "abcdefghijklmnopqrstuvwxyxABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".
            toCharArray();
    ...
    static final Pattern versionRep = Pattern.
            compile("componentversion: *[0-9][0-9.]*",
    ...
    public void uploadRecipe(@NotNull String name, @NotNull Semver version, @NotNull String recipe) {
        ...
        if (c != null) {
            ...
            System.out.println(name + " " + version + " := " + v);
            System.out.println("S4J sat:" + version.satisfies(c.latestVersion().
                    componentVersion())
                    + " lower:" + version.isLowerThan(c.latestVersion().
                            componentVersion())
                    + " greater:" + version.isGreaterThan(c.latestVersion().
                            componentVersion())
                    + " equiv:" + version.isEquivalentTo(c.latestVersion().
                            componentVersion())
            );
        ...
        try {
            uploadRecipe0(recipe);
        } catch (Throwable t) {
            System.out.println(t);
        }
    }
    public void uploadRecipe0(@NotNull String recipe) {
