...
public final class Exec implements Closeable {
    ...
    static {
        ...
        try {
            ...
            error("Error while initializing PATH", ex);
        }
        computePathString();
    }
    private static void error(String e, Throwable ex) {
    ...
    public static boolean successful(boolean ignoreStderr, String command) throws InterruptedException, IOException {
        return new Exec().withExec(command).successful(ignoreStderr);
    }
    ...
    public static Path which(String fn) {  // mirrors shell command
        ...
        for (Path d : paths) {
            Path f = d.resolve(fn);
            if (Files.isExecutable(f)) return f;
        }
        return null;
    }
    ...
    public Optional<Integer> exec() throws InterruptedException, IOException {
        ...
        if (Thread.currentThread().isInterrupted()) {
            throw new InterruptedException("Refusing to execute because the active thread is interrupted");
        }
